...
$employee_id=$db->query_insert("employees",$data);
$data=NULL;
$data["role_id"]=5;
$data["username"]=$_POST["employee_code"];
$data["password"]=md5(str_replace("-","",$_POST["date_of_birth"]));
$data["active"]=1;
$db->query_insert("users",$data);
$data=NULL;
$sheads=$_POST["salary_head_id"];
foreach($sheads as $sid)
{
$data['employee_id']=$employee_id;
 $data["salary_head_id"]=$sid; 
 $db->query_insert("employee_salaries",$data);
}
 $db->close();

 $_SESSION["vsims_msg"]="Employees Added Successfully";
 header("location:employee_offer_letter.php?eid=$employee_id");
?>