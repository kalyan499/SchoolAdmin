...
$sql="select * from students s,courses c,religions t2,castes t3,categories t4,countries t5,states t6 where s.course_id=c.course_id and s.religion_id=t2.religion_id and s.caste_id=t3.caste_id and s.category_id=t4.category_id and s.state_id =t6.state_id and s.country_id=t5.country_id and academic_year='$academic_year' and transport_opted='Yes'".$sql1;
?>
 <div class="row export-section">

                                                    
                                                    <div class="col-md-12 text-right">
                                                        <a href="#" class="btn btn-sm btn-default text-success" onClick ="$('#treport').tableExport({type:'excel',escape:'false'});"><i class="fa fa-file-excel-o"></i></a>
                                                        <a href="#" class="btn btn-sm btn-default text-danger" onClick ="$('#treport').tableExport({type:'pdf',escape:'false'});"><i class="fa fa-file-pdf-o"></i></a>
                                                      <button class="btn btn-sm btn-default" onClick="divp('treport')"><i class="fa fa-print"></i></button>
                                                    </div>
                                                </div>
...
<?php
$sqlh="select * from student_bus_routes sh,bus_routes hd where sh.bus_route_id=hd.bus_route_id and student_id='$student_id' and course_id='$course_id' and year='$year'";
  $resh=$db->query($sqlh);
  $rowh=$db->fetch_array($resh);
  $paid_till_date=0;
 $sqlhf="select sum(fees_paid) as amt from student_payment_transport_fees shf,student_payments sp where sp.student_payment_id=shf.student_payment_id and student_id='$student_id' and course_id='$course_id' and year='$year'";
  $reshf=$db->query($sqlhf);
  if($rowhf=$db->fetch_array($reshf))
  {
    $paid_till_date=$rowhf["amt"];
  }
    $balance=$rowh["route_fees"]-$paid_till_date;
?>
<td><?php echo $rowh["route_name"]; ?></td> 
<td><?php echo $rowh["route_fees"]; ?></td>      
